...
import { Plus, Edit2, Trash2, Eye, Crown, EyeOff, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function MyListingsPage() {
  ...
  const fetchListings = async (pageNum: number) => {
    setLoading(true);
    const res = await fetch(`/api/listings?page=${pageNum}&limit=${itemsPerPage}&mine=true`);
  ...
  useEffect(() => { if (user) fetchListings(page); }, [user, page]);
  ...
  return (
    <div className="max-w-6xl">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-gray-900">My Listings</h1><p className="text-sm text-gray-500 mt-1">{pagination?.total ?? 0} listing{pagination?.total !== 1 && 's'} posted</p></div>
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 text-gray-400 hover:text-primary hover:bg-primary-light rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
